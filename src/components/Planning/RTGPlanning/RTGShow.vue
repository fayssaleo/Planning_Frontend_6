<template>
    ...
    <v-data-table :headers="headers" :items="thisPlanningBoxes"
      ...
      <template v-slot:item="{ item, rowIndex }">
        <tr class="rowToEdit" :class="editMode ? 'clickableCase' : ''">
          ...
          <td width="5%">
            <span class="wh workingHours">{{ Math.trunc(item[0].workingHours) }}</span>
          </td>
          ...
          <v-row v-if="selectedChangeEDSow == 1" class="equipmentEditMaster">
            <v-col v-if="!whModeSee" cols="11" :class="allBothAddButton_switch ? 'speadForEdit' : ''"
              ...
              <v-row>
                ...
                <v-col cols="2" style="margin-bottom: 12px; padding: 0; min-height: 31px"
                  @click="whModeSee = !whModeSee" v-if="allBothAddButton_switch">
                  <div class="cancelEquipment " style="width: 60% !important; cursor:pointer !important" :class="[
                    allBothAddButton_switch ? (whModeSee ? 'cancelXFourHoursOn selectebale' : 'cancelXFourHoursOFF ') : '',
            ...
            <v-col v-else cols="11" :class="allBothAddButton_switch ? 'speadForEdit' : ''" style="margin: 0 auto">
              <v-row>
                ...
                <v-col cols="5" style="margin-bottom: 12px; padding: 0; min-height: 31px"
                  ...
                  <div class="allBothAddButton addD" :class="allBothAddButton_switch ? 'addDChecked cancelX' : ''
                    " v-if="!allBothNextDrivers && !allBothAddButton_delete" @click="openAddEquipment">
                    EDIT {{ getSelected_profile_group.type.toUpperCase() }}(s)
                    <span> </span>
                    <hr width="100%" />
                  </div>
                </v-col>
                      ...
                      <v-row class="custom-scrollbar equipmentEdit" style="
                        ...
                        <table class="timesTable">
                          <tbody>
                            ...
                            <tr v-for="item in equipmentsInPlanningWithCheck" :key="item.id">
                              <td :class="[
                                showSelect ? 'actionHolder' : '',
                                item?.checked && allBothAddButton_delete
                                  ? 'actionHolderDelete'
                        ...
                        ">
                              ...
                              <td v-for="(equipmentOneHour, index) in item.equipmentTimesSelected" :key="item + 'sssq'">
                                <span class="timeClick" :class="equipmentOneHour ? 'timeClick_ed' : ''"
                                  @click="item.equipmentTimesSelected[index] = !item.equipmentTimesSelected[index]">{{
                        ...
                        " class="equipmentForAdd" @click="setEquipmentAdd(null)">
                              ...
                              <td><span class="timeClick">{{ shift__times[0].split("-")[0] }}<br>{{
                                shift__times[0].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick">{{ shift__times[1].split("-")[0] }}<br>{{
                                shift__times[1].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick">{{ shift__times[2].split("-")[0] }}<br>{{
                                shift__times[2].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick">{{ shift__times[3].split("-")[0] }}<br>{{
                                shift__times[3].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick">{{ shift__times[4].split("-")[0] }}<br>{{
                                shift__times[4].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick">{{ shift__times[5].split("-")[0] }}<br>{{
                                shift__times[5].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick">{{ shift__times[6].split("-")[0] }}<br>{{
                                shift__times[6].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick">{{ shift__times[7].split("-")[0] }}<br>{{
                                shift__times[7].split("-")[1]
                              }}</span></td>
                              <td><span class="timeClick"></span></td>
                            </tr>
                          </tbody>
                        </table>
          ...
          <v-card class="dialogHeader pa-2">
            ...
            <div style="padding: 0 5%;">
              <PlanninfAsFileForDownload :actualShift="actualShift" :selectedDate="selectedDate"
                :selectedPlanning="selectedPlanning" :ShiftIndex="ShiftIndex" :thisPlanningBoxes="thisPlanningBoxes"
                :headers="headers" :splited="splited" @splitClicked="splitClicked" />
            </div>
            <v-card-actions class="mt-5"> </v-card-actions>
          </v-card>
        </template>
  ...
  <template>
    <div>
      <vue3-html2pdf :show-layout="false" :enable-download="true" :preview-modal="true" :filename="'ee'"
...
export default {
      ...
      if (this.reloadPlanning) {
        ...
        if (this.getSelected_profile_group.id == 1) {
          if (this.driversInPlanningWithCheck.length > this.equipmentsInPlanningWithCheck.length) {
            ...
            this.fixItForSTSLogic2(equipmentInPlanning_now);
            ...
            let equipmentInPlanning_now = this.equipmentsInThePlanning();
            let trueVal = true;
            this.fixItForSTSLogic(equipmentInPlanning_now, trueVal);
            this.saveDialogShow = true;
            this.ifCheckEditedPlanning = true;
          }
        }
          ...
          let columns__ = 8;
          let stsList__ = [];
          //this.fixItForSTSLogic(equipmentInPlanning_now, trueVal,butyes);
          let equipmentInPlanning_now = this.equipmentsInThePlanning();
          stsList__ = equipmentInPlanning_now.filter((e) => e.equipmentTimesSelected.filter((c) => c).length != 0);
          ...
          let itemsPlanning = [];
          let header = [];
          let equipmentTimesChecked = equipmentInPlanning_now.filter((e) => e.equipmentTimesSelected.filter((c) => c).length != 0);
          header.push({
            text: "Drivers | Time",
            sortable: false,
            key: "Drivers | Time",
          ...
          let butyes = false;
          let butno = true;
          this.fixItForSTSLogic(equipmentInPlanning_now, trueVal, butyes, butno);
          this.saveDialogShow = true;
        }
        ...
        this.fixItForSTSLogic(equipmentInPlanning_now);
        this.saveDialogShow = true;
      }
    },
    fixItForSTSLogic2(equipmentInPlanning_now) {
      console.log("checkEditedPlanning in 2",this.checkEditedPlanning);
      this.ThisOutToBeInserted = [];
      ...
      console.log("this.ThisOutToBeInserted fer:", this.ThisOutToBeInserted);
    },
          ...
          if (this.getSelected_profile_group?.id == 1) {
            ...
            for (let index = 0; index < e.equipmentTimesSelected.length; index++) {
              for (let k = 0; k < this.thisPlanningBoxes.length; k++) {
                if (this.thisPlanningBoxes[k][index + 1] != 'P' && this.thisPlanningBoxes[k][index + 1] != 'DP') {
                  console.log("----------11111----------")
                  console.log("this.thisPlanningBoxes[k][index+1]", this.thisPlanningBoxes[k][index + 1])
                  console.log("e", e)
                  console.log("----------11111----------")
                  if (this.thisPlanningBoxes[k][index + 1].matricule == e.matricule) {
                    console.log("----------2222----------")
  ...
  mounted() {
    this.headers = [];
    console.log("in the RTGShow mounted this.getPlanningWithTheBoxes :",this.getPlanningWithTheBoxes)
    this.thisPlanningBoxes = [
      ...this.getPlanningWithTheBoxes.planning_boxes.slice(1).map((e) => {
        console.log("in the RTGShow mounted e :",e)
